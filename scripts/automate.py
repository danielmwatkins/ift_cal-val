...
def main():
    ...
    complete_cases = get_images(IFT_RESULTS_PATH, VALIDATION_IMG_PATH)
    ...
    for index, row in complete_cases.iterrows():
        pix_conf_mx = image_process(row['manual_path'], row['ift_path'], row['start_date'], row['satellite'], True)
...
def get_images(ift_path, validation_path):
